	...
	public void insertComment() {
		...
		for (long k = 1; k <= 43; k++) {
			...
			for (long i = 1; i <= comments; i++) {
				...
				if (i == 1) { // 게시글의 첫 번째가 댓글의 답변 글이 될 수 없음
					count2 = 0;
				}
				if(depth[count2] == 1) { // 25%의 확률로 rnum(댓글의 개수) 1씩 증가
				    ...
				    rnum++;
				} else if(depth[count2] == 2) { // 75%의 확률로 else if에 진입하면 rnum(댓글 개수)를 0으로 초기화 및 rdepth(댓글 깊이)를 1씩 증가[:wno의 첫 글의 rdepth는 무조건 1 증가]
				    rnum = 0;
				    rdepthVar++;
				    rdepth = rdepthVar;
				}
